...
# write.csv(df_DEI_parametric_mutated,"E:/p33/P33-DEI-dashboard-project/df_DEI_parametric_mutated.csv", row.names = FALSE)
...
unique(df_DEI_demo_long $ Demographics)
...
df_DEI_demo_mutated_long <- df_DEI_demo_long %>%
  ...
  mutate(var_scope = case_when(
    ...
    str_detect(Demographics,"cps") |
      str_detect(Demographics,"chicago naep") ~ "city_chi",
...
df_DEI_demo_tidy <- df_DEI_demo_mutated_long %>%
  rename(note = Demographics,
         demographics = x) %>%
  select(var_yr_ob,demographics,ethnic,var_scope,var_stage,population,note,var_source)

# write.csv(df_DEI_demo_mutated_long,"E:/p33/P33-DEI-dashboard-project/df_DEI_demo_mutated.csv", row.names = FALSE)
