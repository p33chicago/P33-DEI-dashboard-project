...
def metric_DEI_FourG_twoScopes(
    df, metricname, dimension, stage, weight, metricOne, metricTwo, populOne, PopulTwo
):
    ...
    df_clean_fourGroups = df_clean[
        (df_clean["var_ethnic"] != "all")
    ...
    """ extract numerators """
    df_subset = df_clean_fourGroups.copy()[
        (df_clean_fourGroups["metrics_new"] == metricOne)
        | (df_clean_fourGroups["metrics_new"] == metricTwo)
    ]
    ...
    df_popul = df_clean_fourGroups[
        (df_clean_fourGroups["metrics_new"] == populOne)
        | (df_clean_fourGroups["metrics_new"] == PopulTwo)
    ]
    ...
    df_merge = pd.merge(
        ...
        df_popul,
        how="left",
        left_on=["var_scope", "var_ethnic"],
        right_on=["var_scope", "var_ethnic"],
    )
...
def metric_DEI_FourG_oneScope(
    df, metricname, dimension, stage, weight, metricOne, populOne
):
    # CLEAN UP DATA
    """consider metrics with counting numbers only. no proportion"""
    df_clean = df[(df["var_type"] == "count")]
