...
def combiner_CHIAndIL(*df_metric: list[DataFrame]) -> DataFrame:
    ...
    df_metrics_CHI_wide = pd.pivot(df_metrics_CHI_slim,
                                   index=['metric_name', 'var_scope', 'weight', 'dimension', 'stage'],
                                   columns='var_ethnic', values='metric_value')
...
def EI_metric_FourG_geomean(df_metrics_selected: DataFrame) -> DataFrame:
    ...
    df_metrics_selected['avg'] = ((df_metrics_selected['black'] * df_metrics_selected['hispanic'] * df_metrics_selected[
        'white'] * df_metrics_selected['asian']) ** (0.25))
    # caculate the Inequality Index
    ''' II '''
    df_metrics_selected['II_metric'] = ((abs(df_metrics_selected['black'] - df_metrics_selected['avg']) +
...
def EI_indicators_FourG_geomean(df_metrics_selected: DataFrame) -> DataFrame:
    ...
    df_dimension_EI = df_metrics.groupby(by=['var_scope', 'stage', 'dimension']).sum('EI_metric_weighted').drop(
        ['weight'], axis=1)
